...
def fetch_caf_llama_models(config: dict[str, Any]) -> tuple[list[dict], str]:
...
class CafLlamaBotPlugin(CafCliRunPlugin):
    ...
    def _render_connection_fields(self) -> None:
        ...
        dir_col, fetch_col = st.columns([4, 1])
        with dir_col:
            st.text_input(
                "Model Directory",
        ...
        with fetch_col:
            st.write("")
            st.write("")
            if st.button("Fetch", key="btn_caf_llama_fetch_models", use_container_width=True):
                self._fetch_llama_models()
        ...
        if model_names:
            ...
            st.selectbox(
                ...
                help="Fetched from Model Directory. Locks CyberAgentFlow's provider, URL, and model to this managed server.",
            )
        else:
            st.text_input(
                "Model (GGUF)",
                key="caf_llama_srv_model_name",
                help="Model filename/path, or click Fetch above to pick from a scanned list. Locks CyberAgentFlow's provider, URL, and model to this managed server.",
            )
        st.caption(
    ...
    def _fetch_llama_models(self) -> None:
        import streamlit as st
        ...
        with st.spinner("Scanning for .gguf models…"):
            models, error = fetch_caf_llama_models(config)
