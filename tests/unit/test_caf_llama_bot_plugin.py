...
class TestFetchCafLlamaModels:
    def test_no_model_dir_reports_error(self):
        models, error = fetch_caf_llama_models({"execution_target": "local"})
        assert models == []
        assert "Model Directory" in error

    def test_local_missing_directory_reports_error(self, tmp_path):
        missing = tmp_path / "nope"
        models, error = fetch_caf_llama_models({"execution_target": "local", "model_dir": str(missing)})
        assert models == []
        assert "not found" in error

    def test_local_empty_directory_is_not_an_error(self, tmp_path):
        models, error = fetch_caf_llama_models({"execution_target": "local", "model_dir": str(tmp_path)})
        assert models == []
        assert error == ""
    ...
    def test_ssh_missing_directory_reports_distinct_error(self, monkeypatch):
        ...
        models, error = fetch_caf_llama_models({"execution_target": "ssh", "model_dir": "/nope"})
    ...
    def test_ssh_connection_failure_reports_distinct_error(self, monkeypatch):
        ...
        models, error = fetch_caf_llama_models({"execution_target": "ssh", "model_dir": "/models"})
