"""Streamlit AppTest coverage for the CAF + llama.cpp Scan-models UI.
...
This is safe because _render_connection_fields/_scan_llama_models both do a
...
def test_model_directory_has_a_scan_action(real_app_test, tmp_path):
    at = real_app_test.from_function(_render_connection_fields, kwargs={"model_dir": str(tmp_path)})
    at.run()
    assert not at.exception
    assert at.button(key="btn_caf_llama_scan_models")
